(() => {
  ...
  function hasTerminalEicTrailer(value) {
    ...
    const actor = trailer[3].match(/(EIC_AI_SESSION|AGENT|OPERATOR_ACTION|OPERATOR_DECISION|EXTERNAL_SYSTEM|NONE)$/i)?.[1]?.toUpperCase();
  ...
  function messageTurnOwner(node) {
    if (!node?.closest) return node || null;
    const owner = node.closest(CONVERSATION_TURN_SELECTOR);
    if (!owner?.querySelectorAll) return node;
    const roles = [...owner.querySelectorAll("[data-message-author-role]")]
      .map((item) => String(item.getAttribute("data-message-author-role") || "").toLowerCase())
      .filter(Boolean);
    const ownRole = String(node.getAttribute?.("data-message-author-role") || "").toLowerCase();
    // A conversation-turn shell is safe as canonical owner only when it contains
    // one semantic role. This lets one assistant turn own both its reasoning-status
    // fragment and final-answer fragment without ever merging user/assistant turns.
    if (roles.length && roles.every((role) => role === ownRole)) return owner;
    return node;
  }
  ...
  function assistantLifecycleStatusText(value) {
    ...
    if (!text) return true;
    if (/^(?:Tänker|Thinking|Arbetar|Working|Resonerar|Reasoning|Analyserar|Analyzing)(?:\.{0,3}|…)?$/iu.test(text)) return true;
    // ChatGPT can render the completed reasoning-duration disclosure as a separate
    // role-bearing assistant fragment (for example "Arbetade i 51 sekunder >"
    // or "Worked for 2m 54s"). It is page chrome, not the assistant final answer.
  ...
  function stripAssistantPresentationChrome(value) {
    ...
    if (!text) return "";
    // Accessibility/presentation shells may prefix the semantic assistant payload
    // with a localized speaker label. v0.11.21 observed
    // `EIC sade:EIC_TURN: ...`, which invalidated an otherwise exact anchored
    // EIC-AA/5 trailer and caused repair/control churn.
    text = text.replace(
      /^(?:(?:EIC|ChatGPT|Assistant|Assistenten)\s+(?:sade|said)\s*:\s*)/iu,
      ""
    );
    // Some layouts concatenate the completed reasoning-duration disclosure
    // directly with the final answer (`Arbetade i 2m 54sAccepterat...`).
    // Remove exactly one leading presentation disclosure after the known speaker
    // label. Never apply this transform to user-role text.
    text = text.replace(
  ...
  function canonicalMessageEntries() {
    const entries = [];
    ...
    for (const node of document.querySelectorAll("[data-message-author-role]")) {
      ...
      if (node.closest("[data-eic-own-ui='true']")) continue;
      ...
      entries.push({ node, owner, role });
    }
    return entries;
  }

  function messageNodes(role) {
    return canonicalMessageEntries()
  ...
  function canonicalMessageRecord(node) {
    if (!node) {
      return {
        text: "",
        rawTextLength: 0,
        ...
        baselineSchemaVisible: false,
        complete: false
      };
    }
    const role = String(node.getAttribute?.("data-message-author-role") || "").toLowerCase();
    const sourceNode = messageTurnOwner(node) || node;
    const stripAssistantLifecycle = (value) => {
      ...
      return stripAssistantPresentationChrome(
        presentationCleaned.split("\n")
          .filter((line) => !assistantLifecycleStatusText(line))
          .join("\n")
      );
    };
    const rawVisible = stripAssistantLifecycle(sourceNode.innerText || sourceNode.textContent || "");
    const clone = sourceNode.cloneNode?.(true);
    if (clone?.querySelectorAll) {
      for (const control of clone.querySelectorAll(
        "[data-eic-own-ui], script, style, [data-testid*='copy' i], [aria-label*='copy' i]"
      )) {
        control.remove();
      }
      for (const button of clone.querySelectorAll("button")) {
        ...
        if (/^(?:Visa mindre|Visa mer|Show less|Show more|Kopiera kod|Copy code|Copy|Kopiera)$/iu.test(label) ||
            /copy/i.test(button.getAttribute?.("aria-label") || "") ||
            /copy/i.test(button.getAttribute?.("data-testid") || "")) {
          button.remove();
        }
      }
    }
    const cleaned = stripAssistantLifecycle(clone?.textContent || "");
    const rawHasBaseline = /eic\.main-task-baseline\.v2/i.test(rawVisible);
    const cleanedHasBaseline = /eic\.main-task-baseline\.v2/i.test(cleaned);
    const rawMateriallyRicher = rawVisible.length > Math.max(
      cleaned.length + 200,
      Math.floor(cleaned.length * 1.25)
    );
    // Prefer the control-stripped representation normally, but never discard a
    // materially richer rendered response or a structured baseline payload.
    const structuredPayloadPresent = Boolean(sourceNode.querySelector?.("pre, code"));
    const structuredRawRicher = structuredPayloadPresent &&
      rawVisible.length > cleaned.length + 40;
    const text = normalizeMessageText(
      (!cleaned || rawMateriallyRicher || structuredRawRicher || (rawHasBaseline && !cleanedHasBaseline))
        ? rawVisible
        : cleaned
    );
    const baselineSchemaVisible = /eic\.main-task-baseline\.v2/i.test(text);
    const complete = Boolean(text) &&
      (!rawVisible || text === rawVisible || text.length >= Math.floor(rawVisible.length * 0.8)) &&
      (!rawHasBaseline || baselineSchemaVisible);
    return {
      text,
      rawTextLength: rawVisible.length,
      cleanedTextLength: cleaned.length,
      structuredPayloadPresent,
      baselineSchemaVisible,
      complete
    };
  }

  function messageText(node) {
    return canonicalMessageRecord(node).text;
  }

  function getMessageRecords(role) {
    return messageNodes(role).map((node) => ({
      node,
      ...canonicalMessageRecord(node)
  ...
  function getUserMessages() {
    return getMessageRecords("user").map((record) => record.text);
  }

  function getLatestConversationMessage() {
    const entries = canonicalMessageEntries()
      .filter((entry) => (entry.role === "assistant" || entry.role === "user") && Boolean(messageText(entry.node)));
    const entry = entries.at(-1) || null;
    return {
      node: entry?.node || null,
      role: entry?.role || "unknown",
      text: entry ? messageText(entry.node) : ""
    };
  }
  ...
  function getConversationMessages(limit = 10) {
    return canonicalMessageEntries()
      .map((entry) => ({
        role: entry.role,
        text: headTailText(messageText(entry.node), 4000)
      }))
  ...
  async function getPageState({ source = "dom-read" } = {}) {
    const assistantRecords = getMessageRecords("assistant");
    const assistantMessages = assistantRecords.map((record) => record.text);
    const userMessages = getUserMessages();
    const conversationMessages = getConversationMessages();
    const latestMessage = getLatestConversationMessage();
    const latestMessageRole = latestMessage.role;
    const latestAssistantRecord = assistantRecords.at(-1) || null;
    const latestAssistant = latestAssistantRecord?.text || "";
    ...
    const latestAssistantExtractionComplete = latestAssistantRecord?.complete === true;
    const latestAssistantComplete = latestAssistantCandidate &&
      !generating &&
      latestAssistantExtractionComplete;
    ...
    return {
      ...
      latestAssistantComplete,
      latestAssistantExtractionComplete,
      latestAssistantExtraction: latestAssistantRecord ? {
        rawTextLength: latestAssistantRecord.rawTextLength,
        cleanedTextLength: latestAssistantRecord.cleanedTextLength,
        canonicalTextLength: latestAssistant.length,
        structuredPayloadPresent: latestAssistantRecord.structuredPayloadPresent,
        baselineSchemaVisible: latestAssistantRecord.baselineSchemaVisible,
        complete: latestAssistantExtractionComplete
      } : null,
      readAt: Date.now(),
      source
    };
  ...
  function transcriptMessages() {
    return canonicalMessageEntries()
      .map((entry, ordinal) => {
        const { node, owner, role } = entry;
        const reasoning = owner?.querySelector?.("[data-testid*='reasoning' i], [aria-label*='reasoning' i]") ||
          node.querySelector("[data-testid*='reasoning' i], [aria-label*='reasoning' i]");
        const canonical = canonicalMessageRecord(node);
        const text = canonical.text.slice(0, 200000);
        const sourceMessageId = node.getAttribute("data-message-id") || owner?.getAttribute?.("data-turn-id") ||
          owner?.getAttribute?.("data-testid") || node.id || "";
        // Request placeholders and reasoning-duration disclosures are UI lifecycle
        // fragments. canonicalMessageRecord normally removes them from the owning
        // assistant turn; a standalone lifecycle fragment is excluded entirely.
        if (role === "assistant" && (
          /^request-placeholder-/i.test(sourceMessageId) ||
          assistantLifecycleStatusText(text)
        )) return null;
        return {
          role,
          ordinal,
          sourceMessageId,
          text,
          finalAnswer: role === "assistant" ? text : null,
          extraction: {
            rawTextLength: canonical.rawTextLength,
            cleanedTextLength: canonical.cleanedTextLength,
            canonicalTextLength: text.length,
            structuredPayloadPresent: canonical.structuredPayloadPresent,
            baselineSchemaVisible: canonical.baselineSchemaVisible,
          ...
          reasoningSummary: reasoning
            ...
            : "",
          links: [...((owner || node).querySelectorAll?.("a[href]") || [])].map((link) => link.href).slice(0, 100),
          attachments: [...((owner || node).querySelectorAll?.("[data-testid*='attachment' i], [aria-label*='attachment' i]") || [])]
            .map((item) => ({
    ...
    try {
      ...
      for (let sweep = 0; sweep < 80; sweep += 1) {
        ...
        for (const message of visible) {
          // When ChatGPT exposes no stable DOM message id, preserve occurrences by
          // canonical DOM ordinal plus the full message content. v0.12.8 used only
          // role + first 240 characters + total length, which collapsed legitimate
          // repeated turns and prefix collisions.
          const key = message.sourceMessageId ||
            `${message.role}:${Number(message.ordinal || 0)}:${message.text}`;
  ...
  async function hydrateLinkStatusFromOwner() {
    try {
      const response = await chrome.runtime.sendMessage({
        type: "EIC_CONTENT_BRIDGE_READY",
        version: VERSION,
        documentEpoch: DOCUMENT_EPOCH,
        conversationKey: conversationKey()
      });
      if (response?.ok && response.status) {
        setLinkStatus(response.status, response.detail || "", response.overlay || null);
      }
    } catch {
      // Durable background state remains owner; failed hydration is retried on the next bridge lifecycle.
    }
  }
  ...
  startObserver();
  void hydrateLinkStatusFromOwner();
