  ...
  const controllerValue = pair.surfaces[MISSION_SURFACE_ROLES.CHATGPT_CONTROLLER] ||
    (legacyRecord ? {
      ...
      lifecycleState: legacyRecord.status === "UNSUPPORTED"
        ...
        : SURFACE_LIFECYCLE_STATES.READY,
      active: nullableInteger(legacyRecord.tabId) !== null && nullableInteger(legacyRecord.tabId) === nullableInteger(selectedTabId),
...
export function bindSurfaceRole(pairValue, role, {
  ...
  if (!tab || nullableInteger(tab.id) === null) throw new Error("SURFACE_TAB_INVALID");
  ...
  if (nullableInteger(pair.surfaces[otherRole]?.tabId) !== null && nullableInteger(pair.surfaces[otherRole]?.tabId) === nullableInteger(tab.id)) {
    throw new Error(`SURFACE_ROLE_TAB_CONFLICT:${tab.id}`);
  }
  const previous = pair.surfaces[role];
  const surface = normalizeSurface({
    ...previous,
    tabId: nullableInteger(tab.id),
    windowId: nullableInteger(tab.windowId ?? pair.windowId),
...
export function reconcileControllerSurfacePage(pairValue, {
  tab,
  page,
  now = Date.now()
} = {}) {
  if (!tab || nullableInteger(tab.id) === null || !page?.documentEpoch) return pairValue;
  const pair = normalizeSurfacePair(pairValue, {
    windowId: tab.windowId ?? pairValue?.windowId,
    now
  });
  const role = findSurfaceRoleByTab(pair, tab.id);
  if (role !== MISSION_SURFACE_ROLES.CHATGPT_CONTROLLER) return pair;
  validateRoleUrl(role, page.url || tab.url || "");
  const previous = pair.surfaces[role];
  const nextUrl = text(page.url || tab.url || previous.url, 4000);
  const nextOrigin = safeOrigin(nextUrl);
  const nextConversationKey = text(
    page.conversationKey || conversationKeyFromUrl(nextUrl),
    1000
  );
  const nextDocumentEpoch = String(page.documentEpoch || "");
  const materialChanged =
    previous.url !== nextUrl ||
    previous.origin !== nextOrigin ||
    previous.conversationKey !== nextConversationKey ||
    previous.documentEpoch !== nextDocumentEpoch ||
    previous.lifecycleState !== SURFACE_LIFECYCLE_STATES.READY ||
    previous.title !== text(page.title || tab.title || previous.title, 500);

  if (!materialChanged) return pair;

  pair.surfaces[role] = normalizeSurface({
    ...previous,
    title: page.title || tab.title || previous.title,
    url: nextUrl,
    origin: nextOrigin,
    conversationKey: nextConversationKey,
    lifecycleState: SURFACE_LIFECYCLE_STATES.READY,
    documentEpoch: nextDocumentEpoch,
    epochRevision: Number(previous.epochRevision || 0) +
      (previous.documentEpoch !== nextDocumentEpoch ? 1 : 0),
    active: tab.active ?? previous.active,
    discarded: tab.discarded ?? previous.discarded,
    frozen: tab.frozen ?? previous.frozen,
    lastReason: "CONTENT_PAGE_RECONCILED",
    lastSeenAt: nowIso(now),
    updatedAt: nowIso(now)
  }, role, { windowId: pair.windowId, now });
  pair.revision += 1;
...
export function replaceSurfaceTab(pairValue, removedTabId, addedTab, {
  ...
  if (!addedTab || nullableInteger(addedTab.id) === null) throw new Error("SURFACE_REPLACEMENT_TAB_INVALID");
  const pair = normalizeSurfacePair(pairValue, {
  ...
  pair.surfaces[role] = normalizeSurface({
    ...previous,
    tabId: nullableInteger(addedTab.id),
    windowId: nullableInteger(addedTab.windowId ?? previous.windowId),
...
export function setSurfaceActiveTab(pairValue, activeTabId, {
  ...
  for (const role of ROLE_SET) {
    ...
    if (!surface) continue;
    const activeId = nullableInteger(activeTabId);
    const active = activeId !== null && nullableInteger(surface.tabId) === activeId;
...
export function findSurfaceRoleByTab(pairValue, tabId) {
  ...
  for (const role of ROLE_SET) {
    const candidateTabId = nullableInteger(pairValue.surfaces[role]?.tabId);
    const requestedTabId = nullableInteger(tabId);
    if (requestedTabId !== null && candidateTabId === requestedTabId) return role;
  }
  return null;
}
