...
export async function synthesizeSessionMemory({
  ...
  const normalizedConversationKey = sanitizeText(conversationKey, 1000);
  const normalizedTaskFingerprint = sanitizeText(taskFingerprint, 500);
  const normalizedBranchKey = sanitizeText(branchKey, 1000);
  const captureId = sanitizeText(capture?.id, 300);
  const captureTurnIds = new Set((capture?.turnIds || []).map(String));

  // v0.11.19: Session Memory is a projection of the current capture, never a
  // cross-conversation accumulator. A prior memory may preserve chronology
  // metadata only when it belongs to the exact same conversation/task/branch;
  // its section summaries are intentionally not merged because the current
  // cumulative capture is re-sectioned in full.
  const priorCompatible = Boolean(
    priorMemory &&
    priorMemory.conversationKey === normalizedConversationKey &&
    priorMemory.taskFingerprint === normalizedTaskFingerprint &&
    (!normalizedBranchKey || !priorMemory.branchKey || priorMemory.branchKey === normalizedBranchKey)
  );
  const summaries = (Array.isArray(sectionSummaries) ? sectionSummaries : []).filter((summary) => {
    if (!summary || summary.conversationKey !== normalizedConversationKey) return false;
    if (captureId && sanitizeText(summary.captureId, 300) !== captureId) return false;
    const ids = Array.isArray(summary.sourceTurnIds) ? summary.sourceTurnIds.map(String) : [];
    return ids.every((id) => captureTurnIds.has(id));
  });
  const invalidSummaryCount = Math.max(0, (sectionSummaries || []).length - summaries.length);
  const registers = mergeRegisters(summaries);
  const sourceHashes = [...new Set(summaries.flatMap((summary) => summary.sourceHashes || []))];
  const sourceTurnIds = [...new Set(summaries.flatMap((summary) => summary.sourceTurnIds || []).map(String))];
  const provenanceValid = invalidSummaryCount === 0 &&
    sourceTurnIds.every((id) => captureTurnIds.has(id));
  const id = sanitizeText(memoryId, 300) ||
    `memory-${(await sha256Hex(`${normalizedConversationKey}:${normalizedTaskFingerprint}:${sourceHashes.join(":")}`)).slice(0, 32)}`;
  return {
    ...
    state: provenanceValid ? SESSION_MEMORY_STATES.FRESH : SESSION_MEMORY_STATES.RESET_REQUIRED,
    staleReasons: provenanceValid ? [] : ["MEMORY_PROVENANCE_MISMATCH"],
    ...
    ownerTruth: false,
    createdAt: priorCompatible ? priorMemory.createdAt : nowIso(),
    updatedAt: nowIso()
  };
}
...
export function evaluateSessionMemoryFreshness(memory, context = {}) {
  ...
  const reasons = [];
  if (memory.state === SESSION_MEMORY_STATES.RESET_REQUIRED) {
    reasons.push(...(Array.isArray(memory.staleReasons) && memory.staleReasons.length
      ? memory.staleReasons
      : ["MEMORY_RESET_REQUIRED"]));
  } else if (memory.state === SESSION_MEMORY_STATES.STALE) {
    reasons.push(...(Array.isArray(memory.staleReasons) && memory.staleReasons.length
      ? memory.staleReasons
      : ["MEMORY_MARKED_STALE"]));
  }
  if (Number(memory?.provenance?.invalidSummaryCount || 0) > 0) {
    reasons.push("MEMORY_PROVENANCE_MISMATCH");
  }
