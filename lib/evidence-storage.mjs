...
import { nullableInteger, sha256Hex, stableStringify } from "./common.mjs";
...
function requiredWindowId(value) {
  const windowId = nullableInteger(value);
  if (windowId === null) throw new Error("EVIDENCE_WINDOW_ID_REQUIRED");
  return windowId;
}
...
function screenshotBodyKey(windowId, itemId) {
  return `${BODY_PREFIX}.${requiredWindowId(windowId)}.${String(itemId)}`;
}
...
async function saveRootReadback(chromeApi, root, windowId, expectedRevision) {
  ...
  const stored = readback.windows[String(requiredWindowId(windowId))];
...
export async function loadWindowEvidenceStore(chromeApi, windowId) {
  ...
  return normalizeEvidenceStore(root.windows[String(requiredWindowId(windowId))], { windowId });
}
...
export async function clearWindowEvidence(chromeApi, windowId) {
  const root = await localRoot(chromeApi);
  const key = String(requiredWindowId(windowId));
