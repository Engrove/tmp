...
/**
 * v0.12.13 single ownership verdict.
 *
 * v0.12.12 asked two different questions about the same effect in two places.
 * `newResponseAdmissionReady` consulted only the legacy journal, which has no
 ...
 * acknowledged. `effectReady` consulted causal control, where RESPONSE_CONSUME
 * closes the effect and clears `activeEffectId`. Both were locally correct and
 * permanently contradictory: gate one reported admission=1 while gate two
 * refused to admit a candidate, and because gate one also suppresses the
 * autonomous recovery block, the pair formed a live-lock with no producer.
 *
 * This is the only predicate either gate may ask. Causal control is the source
 * of truth for ownership; the legacy journal supplies the effect identity.
 */
export const CAUSAL_OWNERSHIP_VERDICT = Object.freeze({
  NO_EFFECT: "NO_EFFECT",
  OWNER_ACTIVE: "OWNER_ACTIVE",
  OWNER_NOT_ACKED: "OWNER_NOT_ACKED",
  OWNER_UNREGISTERED: "OWNER_UNREGISTERED",
  OWNER_SUCCEEDED: "OWNER_SUCCEEDED",
  OWNER_CONSUMED_WITHOUT_SUCCESSOR: "OWNER_CONSUMED_WITHOUT_SUCCESSOR",
  OWNER_TERMINAL_WITHOUT_SUCCESSOR: "OWNER_TERMINAL_WITHOUT_SUCCESSOR"
});

const CAUSAL_OWNERSHIP_DESYNC = new Set([
  CAUSAL_OWNERSHIP_VERDICT.OWNER_CONSUMED_WITHOUT_SUCCESSOR,
  CAUSAL_OWNERSHIP_VERDICT.OWNER_TERMINAL_WITHOUT_SUCCESSOR
]);

export function classifyCausalOwnership(run = {}, legacyEffect = null) {
  const legacyStatus = text(legacyEffect?.status, 80);
  const effectId = text(legacyEffect?.effectId, 180);
  const base = { effectId, legacyStatus, causalStatus: "", closeReason: "" };
  if (!legacyEffect || !effectId) {
    return { ...base, verdict: CAUSAL_OWNERSHIP_VERDICT.NO_EFFECT, admissible: true, desynced: false };
  }

  const state = normalizeCausalControlState(run.causalControl, { runId: run.runId });
  const causal = state.effects.find((item) => item.effectId === effectId) || null;
  const causalStatus = text(causal?.status, 80);
  const closeReason = text(causal?.closeReason, 240);
  const active = state.activeEffectId
    ? state.effects.find((item) => item.effectId === state.activeEffectId) || null
    : null;
  const detail = { ...base, causalStatus, closeReason };

  if (active && active.effectId === effectId) {
    const acked = active.status === CAUSAL_EFFECT_STATUS.ACKED;
    return {
      ...detail,
      verdict: acked ? CAUSAL_OWNERSHIP_VERDICT.OWNER_ACTIVE : CAUSAL_OWNERSHIP_VERDICT.OWNER_NOT_ACKED,
      admissible: acked,
      desynced: false
    };
  }

  // A different live effect owns the run: the turn advanced normally and this
  // legacy row is history, not a stranded owner.
  if (active) {
    return { ...detail, verdict: CAUSAL_OWNERSHIP_VERDICT.OWNER_SUCCEEDED, admissible: false, desynced: false };
  }

  // Never registered causally yet. The tick preflight registers it; transient.
  if (!causal) {
    return { ...detail, verdict: CAUSAL_OWNERSHIP_VERDICT.OWNER_UNREGISTERED, admissible: false, desynced: false };
  }

  // No live owner anywhere and this effect is terminal, yet the legacy journal
  // still presents it as live. This is the stranded state: nothing owns the
  // next step, so nothing can ever admit the response sitting in the DOM.
  if (TERMINAL_EFFECT.has(causal.status) || causal.status === CAUSAL_EFFECT_STATUS.RESPONSE_CONSUMED) {
    const consumed = causal.status === CAUSAL_EFFECT_STATUS.RESPONSE_CONSUMED ||
      closeReason === "RESPONSE_CONSUMED" ||
      Boolean(causal.responseConsumedAt);
    return {
      ...detail,
      verdict: consumed
        ? CAUSAL_OWNERSHIP_VERDICT.OWNER_CONSUMED_WITHOUT_SUCCESSOR
  ...
  return { ...detail, verdict: CAUSAL_OWNERSHIP_VERDICT.OWNER_NOT_ACKED, admissible: false, desynced: false };
}

export function causalOwnershipDesynced(verdict = "") {
  return CAUSAL_OWNERSHIP_DESYNC.has(text(verdict, 80));
}
