import { deepClone, nullableInteger, sanitizeText } from "./common.mjs";
...
export function bindContinuityScope(value, {
  ...
  const numericWindowId = nullableInteger(windowId, { min: 0 });
  const validWindowId = numericWindowId !== null;
  continuity.scope = {
...
export function continuityScopeWindowId(value) {
  ...
  return nullableInteger(raw, { min: 0 });
}
  ...
  if (context?.continuity && typeof context.continuity === "object") {
    return {
      ...
      changed: context.continuity?.scope?.kind !== CONTINUITY_SCOPE_KIND ||
        nullableInteger(context.continuity?.scope?.windowId, { min: 0 }) !==
          nullableInteger(windowId, { min: 0 }),
      adoptedRoot: false,
...
export function promoteContinuityConversation(value, {
  ...
  const changedConversation = Boolean(
    (current && current !== next) ||
    (!current && prior && prior !== next)
  );
  if (changedConversation) {
    // v0.11.19: intent, work-unit, baseline, claims and anti-loop state are
    // conversation-owned. Rebinding only the locator made stale task context look
    // current and allowed a new chat to bypass its mandatory fresh baseline.
    ...
    fresh.position.conversationKey = next;
    fresh.position.taskFingerprint = sanitizeText(taskFingerprint, 256);
    fresh.position.updatedAt = new Date().toISOString();
    fresh.conversationRebase = {
      from: current || prior,
      to: next,
      reason: "CONVERSATION_CHANGED_RESET_TASK_CONTEXT",
      at: fresh.position.updatedAt
    };
    return { continuity: fresh, changed: true, reason: "CONVERSATION_REBASED" };
  }
