import { normalizeWhitespace, sanitizeText, sha256Hex } from "./common.mjs";
...
export async function validateEffectPayloadIntegrity(effect) {
  const prompt = effect?.prompt;
  const expectedDigest = sanitizeText(effect?.promptDigest, 128).toLowerCase();
  if (typeof prompt !== "string" || prompt.length === 0) {
    return {
      valid: false,
      code: "EFFECT_PAYLOAD_MISSING",
      expectedDigest,
      recomputedDigest: ""
    };
  }
  if (!/^[a-f0-9]{64}$/u.test(expectedDigest)) {
    return {
      valid: false,
      code: "EFFECT_PAYLOAD_IDENTITY_MISSING",
      expectedDigest,
      recomputedDigest: ""
    };
  }
  const recomputedDigest = await sha256Hex(prompt);
  if (recomputedDigest !== expectedDigest) {
    return {
      valid: false,
      code: "EFFECT_PAYLOAD_DIGEST_MISMATCH",
      expectedDigest,
      recomputedDigest
    };
  }
