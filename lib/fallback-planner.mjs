...
export function repairDeterministicDecision(decisionValue = {}, {
  ...
  const localAgentOwner = ownerRoute === AUTONOM_AGENT_LOCAL_SESSION_STATE;
  if (localAgentOwner && (needsActionRepair || needsContextRepair)) {
    // v0.12.3: AUTONOM_AGENT_LOCAL_SESSION_STATE is an internal owner label for
    // the already-running Chrome extension, not a tool/API available to the
    // ChatGPT AI session. Rewriting a meta continuation into "call this route"
    // taught the target AI the wrong actor model and could deadlock bootstrap.
    decision.requestedAction = sanitizeText(
      `EIC Autonom Agent är redan den aktiva lokala Chrome-extensionruntime för denna session. ` +
      `Försök inte starta eller anropa Agenten, GPT Actions, verktyg, API:er eller ${AUTONOM_AGENT_LOCAL_SESSION_STATE}. ` +
      `Svara i denna ChatGPT-session med det innehåll som efterfrågas; Agenten observerar svaret lokalt. ` +
      `Fortsätt därefter med: ${requested}`,
      5000
    );
  } else if (needsActionRepair) {
    decision.requestedAction = sanitizeText(
  ...
  decision.requiredEvidence = uniqueText([
    ...
    localAgentOwner
      ? "Det efterfrågade chatsvaret observeras av den redan aktiva lokala EIC Autonom Agent-extensionen; ingen AI-side owner-route invocation krävs."
  ...
  decision.deterministicRepair = {
    code: localAgentOwner && (needsActionRepair || needsContextRepair)
      ? "LOCAL_AGENT_CHAT_RESPONSE_REPAIR"
      : needsActionRepair
        ? "META_ONLY_TO_OWNER_READ"
        : "GROUNDING_CONTEXT_REPAIR",
...
export function buildDeterministicDecision({
  ...
  if (observation?.mainTaskBaselineRequired === true) {
    ...
    return {
      ...
      omissionFailure: "Utan ett strukturerat eic.main-task-baseline.v3-svar saknar Nano stabilt huvudmål, avgränsad arbetsenhet och nästa högvärdesåtgärd.",
      unlocksNextAction: "Analysera det returnerade eic.main-task-baseline.v3-svaret med Nano och slutför sessionsinitieringen.",
      boundedStop: false,
      ...
      requiredEvidence: [
        "Ett komplett JSON-objekt enligt eic.main-task-baseline.v3 följt av EIC-AA/5, levererat som vanligt chatsvar i den anslutna sessionen."
      ],
      verifiedFacts: [],
      ...
      alternatives: [],
      continueCriteria: [
        "Ett komplett eic.main-task-baseline.v3-svar har observerats och validerats lokalt av Agenten."
      ],
      stopCriteria: [
        "Ett validerat eic.main-task-baseline.v3-svar finns för den aktuella sessionen.",
      ...
      boundaryEvidence: "",
      unlockEvent: "Ett komplett validerat eic.main-task-baseline.v3-svar observeras i den anslutna ChatGPT-sessionen.",
