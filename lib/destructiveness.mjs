...
// Control-protocol markers are not lexical effect hazards. `USER_PAUSE` is
// authoritative only through the structured `pauseOrigin` branch above. Keeping the
// token in this lexical list caused test/verification instructions that merely named
// USER_PAUSE to be misclassified as a level-10 human-authority effect.
const LEVEL_10 = [
  /\bCAPTCHA\b/i,
...
export const CLASSIFIER_VERSION = "EIC_DESTRUCTIVENESS/1@0.12.0";
...
export function resolveAutonomousPause({
  ...
  const alternative = Array.isArray(decision.alternatives)
    ...
    : "";
  const fallback = sanitizeText(fallbackAction, 5000) ||
    "Läs om den exakta owner-statusen och fortsätt med minsta säkra, reversibla nästa steg.";

  // Protected session-init owns a canonical baseline/correction action. A valid
  // target CONTINUE marker, no-progress recovery or Hjalmar metadata may inform
  // control, but it must never replace that canonical action after it is sealed.
  if (protectedSessionInit) {
    return {
      action: decision?.action === "PAUSE" ? "PAUSE" : "CONTINUE",
      humanRequired: false,
      reason: control?.verdict === "READ_REQUIRED"
        ? "HJALMAR_MENTAL_CONTROL_READ_REQUIRED_PROTECTED_SESSION_INIT"
        : "PROTECTED_SESSION_INIT_CANONICAL_ACTION_PRESERVED",
      requestedAction: requested || alternative || fallback
    };
  }

  const base = targetNext || requested || alternative || fallback;
