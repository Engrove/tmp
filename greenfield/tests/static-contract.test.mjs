...
test("v1.7.6 identity is consistent across runtime surfaces", () => {
  ...
  assert.match(contracts, /APP_VERSION = "1\.7\.6"/);
  assert.match(content, /const CONTENT_VERSION = "1\.7\.6"/);
  assert.match(content, /EIC Greenfield v\$\{CONTENT_VERSION\}/);
  assert.match(sidepanel, /appVersion: "1\.7\.6"/);
  assert.match(panelHtml, /Greenfield <span>v1\.7\.6<\/span>/);
});
