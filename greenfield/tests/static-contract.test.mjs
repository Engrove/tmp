...
test("v1.8.7 identity is consistent across runtime surfaces", () => {
  ...
  assert.match(contracts, /APP_VERSION = "1\.8\.7"/);
  assert.match(content, /const CONTENT_VERSION = "1\.8\.7"/);
  assert.match(content, /EIC Greenfield v\$\{CONTENT_VERSION\}/);
  assert.match(sidepanel, /appVersion: "1\.8\.7"/);
  assert.match(panelHtml, /Greenfield <span>v1\.8\.7<\/span>/);
});
