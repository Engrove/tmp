...
test("v1.8.3 identity is consistent across runtime surfaces", () => {
  ...
  assert.match(contracts, /APP_VERSION = "1\.8\.3"/);
  assert.match(content, /const CONTENT_VERSION = "1\.8\.3"/);
  assert.match(content, /EIC Greenfield v\$\{CONTENT_VERSION\}/);
  assert.match(sidepanel, /appVersion: "1\.8\.3"/);
  assert.match(panelHtml, /Greenfield <span>v1\.8\.3<\/span>/);
});
