...
test("v1.8.1 version identity is coherent across executable package surfaces", () => {
  const manifest = JSON.parse(read("manifest.json"));
  const pkg = JSON.parse(read("package.json"));
  const contracts = read("lib/contracts.mjs");
  const content = read("content.js");
  const sidepanel = read("sidepanel.html");
  assert.equal(read("VERSION").trim(), "1.8.1");
  assert.equal(manifest.version, "1.8.1");
  assert.equal(pkg.version, "1.8.1");
  assert.match(contracts, /APP_VERSION\s*=\s*"1\.8\.1"/);
  assert.match(content, /CONTENT_VERSION\s*=\s*"1\.8\.1"/);
  assert.match(sidepanel, /Greenfield\s*<span>v1\.8\.1<\/span>/);
});
