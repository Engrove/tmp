 ...
 */
/**
 * v0.7.1 raises the character ceiling and demotes it from fatal to advisory.
 *
 * v0.7.0 set it to 6 000 and threw on breach. Field run
 * run-b175d216-8fba-4fa8-8b82-2a367fb39b90 then produced legitimate decisions of 5 999
 * and 5 969 characters and had one aborted at 6 003 after 520 s of inference — a 0.05 %
 * margin. The bounded DECISION_SCHEMA permits roughly 36 000 characters worst case, so
 * 6 000 was not derived from the contract it was supposed to protect.
 *
 * The runaway this ceiling exists to stop had a different signature: 10 985 characters
 * across 6 647 chunks under the *unbounded* v0.6.9 schema. Chunk count is the reliable
 * discriminator, so it stays strict while the character ceiling gets real headroom, and
 * a breach first tries to recover a complete JSON object from what already streamed.
 */
const NANO_MAX_OUTPUT_CHARS = 12_000;
const NANO_MAX_OUTPUT_CHUNKS = 4_000;
...
const DECISION_SCHEMA = {
  type: "object",
  required: [
    "analysisMode", "intent", "action", "progressDelta", "reason", "workUnit",
    "primaryProgramGoal", "activeMilestone", "boundedCurrentUnit", "directProgramDelta",
    "requiredControl", "omissionFailure", "unlocksNextAction", "evidenceClass", "completionState", "boundedStop",
    "requestedAction", "requiredEvidence", "targetClaims",
    "inferences", "contextEvidence", "completionScope", "blockers", "alternatives", "completionConfirmed",
    "pauseOrigin", "boundaryEvidence", "destructivenessLevel",
    "destructivenessRationale", "rollbackPath", "readbackPlan", "materialAmbiguity", "trackControl"
  ],
  properties: {
    analysisMode: { type: "string", enum: ["TAKEOVER_BOOTSTRAP", "CONTINUATION_ANALYSIS", "OPERATOR_RESUME"] },
    intent: { type: "string", minLength: 1, maxLength: 1200 },
    action: { type: "string", enum: ["CONTINUE", "DONE", "PAUSE"] },
    progressDelta: { type: "integer", minimum: 0, maximum: 3 },
    primaryProgramGoal: { type: "string", minLength: 1, maxLength: 1200 },
    activeMilestone: { type: "string", minLength: 1, maxLength: 800 },
    boundedCurrentUnit: { type: "string", minLength: 1, maxLength: 800 },
    directProgramDelta: { type: "integer", minimum: 0, maximum: 3 },
    requiredControl: { type: "boolean" },
    omissionFailure: { type: "string", maxLength: 800 },
    unlocksNextAction: { type: "string", maxLength: 800 },
    evidenceClass: {
      type: "string",
      enum: ["OWNER_LIVE", "OWNER_RECEIPT", "OWNER_HISTORICAL", "DERIVED_VIEW", "LOCAL_CANDIDATE", "ROUTING_CONTEXT", "DRY_RUN"]
    },
    completionState: {
      type: "string",
      enum: ["UNIT_DONE", "MILESTONE_CONTINUE", "PROGRAM_BLOCKED", "PROGRAM_DONE"]
    },
    boundedStop: { type: "boolean" },
    reason: { type: "string", minLength: 1, maxLength: 1200 },
    workUnit: { type: "string", minLength: 1, maxLength: 800 },
    requestedAction: { type: "string", maxLength: 5000 },
    requiredEvidence: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 6 },
    verifiedFacts: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 6 },
    targetClaims: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 6 },
    inferences: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 6 },
    evidenceAnchors: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 6 },
    contextEvidence: { type: "array", items: { type: "string", maxLength: 400 }, minItems: 1, maxItems: 6 },
    attempts: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 4 },
    blockers: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 4 },
    alternatives: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 4 },
    continueCriteria: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 4 },
    stopCriteria: { type: "array", items: { type: "string", maxLength: 400 }, maxItems: 4 },
    completionEvidence: { type: "string", maxLength: 800 },
    completionScope: { type: "string", enum: ["WORK_UNIT", "STABLE_GOAL"] },
    completionConfirmed: { type: "boolean" },
    pauseOrigin: {
      type: "string",
      enum: [
        "NONE", "TARGET_REQUESTED_PAUSE", "USER_PAUSE", "MODEL_UNCERTAINTY",
        "EVIDENCE_BOUNDARY", "AUTHORIZATION_BOUNDARY",
        "AUTHENTICATION_OR_CAPTCHA", "DESTRUCTIVE_CONFIRMATION_REQUIRED",
        "PRIVACY_OR_SECRET_BOUNDARY", "POLICY_OR_SAFETY_BOUNDARY",
        "NANO_HOST_REQUIRED", "PROTOCOL_MISSING",
        "NO_PROGRESS", "GENUINE_DEAD_END", "OPERATOR_PROXY_CANDIDATE"
      ]
    },
    boundaryEvidence: { type: "string", maxLength: 600 },
    destructivenessLevel: { type: "integer", minimum: 1, maximum: 10 },
    destructivenessRationale: { type: "string", maxLength: 600 },
    rollbackPath: { type: "string", maxLength: 400 },
    readbackPlan: { type: "string", maxLength: 400 },
    materialAmbiguity: { type: "string", enum: ["NONE", "PRESENT", "UNKNOWN"] },
    ownerRoute: { type: "string", maxLength: 400 },
    exactTarget: { type: "string", maxLength: 400 },
    unlockEvent: { type: "string", maxLength: 400 },
    hjalmarVerdict: { type: "string", maxLength: 200 },
    hjalmarEvidenceLimit: { type: "string", maxLength: 400 },
    trackControl: {
      type: "object",
      required: [
        "status", "baselinePresent", "currentActionRelation", "eightyTwentyVerdict",
        "activeGlobalSkills", "requiredGlobalSkills", "skillGaps",
        "detourReason", "returnCondition", "correctionPrompt"
      ],
      properties: {
        status: { type: "string", enum: ["BASELINE_REQUESTED", "ON_TRACK", "JUSTIFIED_DETOUR", "DRIFT", "INSUFFICIENT_CONTEXT"] },
        baselinePresent: { type: "boolean" },
        currentActionRelation: { type: "string", enum: ["DIRECT", "REQUIRED_DETOUR", "UNRELATED", "UNKNOWN"] },
        eightyTwentyVerdict: { type: "string", enum: ["HIGH_LEVERAGE", "NECESSARY_ENABLER", "LOW_LEVERAGE", "UNKNOWN"] },
        activeGlobalSkills: {
          type: "array", maxItems: 16,
          items: { type: "object", properties: {
            code: { type: "string", maxLength: 160 },
            reason: { type: "string", maxLength: 600 },
            payloadHash: { type: "string", maxLength: 160 }
          } }
        },
        ...
        detourReason: { type: "string", maxLength: 1000 },
        returnCondition: { type: "string", maxLength: 1000 },
        correctionPrompt: { type: "string", maxLength: 5000 }
      }
    },
    operatorCandidate: {
      type: "object",
      properties: {
        triggerType: { type: "string", enum: ["NONE", "HUMAN_ACTION_CANDIDATE", "OPERATOR_PROXY_REQUIRED"] },
        actionCode: { type: "string", maxLength: 120 },
        exactTarget: { type: "string", maxLength: 400 },
        proposedAction: { type: "string", maxLength: 800 },
        expectedEffect: { type: "string", maxLength: 800 },
        ownerSurface: { type: "string", maxLength: 400 },
        ownerEvidenceLocator: { type: "string", maxLength: 400 },
        reversibility: { type: "string", enum: ["YES", "NO", "UNKNOWN"] },
        rollbackPath: { type: "string", maxLength: 400 },
        readbackPlan: { type: "string", maxLength: 400 },
        destructivenessLevel: { type: "integer", minimum: 1, maximum: 10 },
        destructivenessRationale: { type: "string", maxLength: 600 },
        humanAuthorityClass: { type: "string", enum: ["NOT_REQUIRED", "REQUIRED", "UNKNOWN"] },
        materialAmbiguity: { type: "string", enum: ["NONE", "PRESENT", "UNKNOWN"] }
      }
    }
  }
...
const elements = Object.fromEntries([
  ...
  "captureSessionButton", "fullCaptureSessionButton", "purgeSessionContextButton",
...
function uiConfig() {
  ...
  return {
    ...
    nanoMandateVersion: elements.nanoMandateVersion.value || "nano-core-v7",
    targetMandate: elements.targetMandate.value,
    targetMandateProfile: elements.targetMandateProfile.value || "CUSTOM",
    targetMandateVersion: elements.targetMandateVersion.value || "target-core-v6",
...
function renderNano() {
  ...
  if (state.modelStale && !hostActivationBusy) {
    setBadge(elements.nanoBadge, "OMSTART KRÄVS", "warning");
...
function renderNanoPipeline(run) {
  ...
  const duration = Number(active?.durationMs || telemetry.lastDurationMs || (
    startedAt ? Math.max(0, (completedAt || Date.now()) - startedAt) : NaN
  ));
  elements.nanoDurationStatus.textContent = Number.isFinite(duration)
    ? formatDuration(duration)
    : "—";

  const inputChars = Number(active?.inputChars || telemetry.lastInputChars || 0);
  const inputDigest = active?.inputDigest || telemetry.lastInputDigest || "";
  elements.nanoInputStatus.textContent = inputChars
    ? `${inputChars.toLocaleString("sv-FI")} tecken · ${inputDigest.slice(0, 12)}…`
    : "—";

  const outputChars = Number(active?.outputChars || telemetry.lastOutputChars || 0);
  const chunkCount = Number(active?.chunkCount || telemetry.lastChunkCount || 0);
  const result = active?.resultSummary || telemetry.lastResultSummary || telemetry.lastError || "";
  elements.nanoResultStatus.textContent = active?.status === "RUNNING"
  ...
  if (active?.status === "RUNNING") {
    ...
    elements.nanoDetail.textContent = "Nano-request är exakt claimad, stream-/heartbeat-bevakad och kan inte ersättas av deterministic fallback under aktiv inferens.";
  } else if (state.modelStatus === "available") {
...
function renderBrowserRecovery(windowContext = {}, buildProfile = {}, evidence = {}) {
  ...
  const exactIdentity = Boolean(
    Number.isInteger(Number(target.tabId)) &&
    target.surfaceId &&
...
function renderOperatorAction(run) {
  ...
  elements.operatorActionEvidence.textContent = String(action.expectedEvidence || "Verifierat slutförandekvitto");
...
function renderSessionContextInitFailure(run) {
  ...
  elements.retrySessionContextInitButton.disabled = !overlay.retryable;
  elements.retrySessionContextInitButton.title = overlay.retryable
    ? "Armera om catch, baselinefråga och Nano-analys från fas 1."
...
function renderControls(run, hasTarget, windowContext = {}) {
  ...
  const hard = Boolean(run && [STATES.PROGRAM_BLOCKED, STATES.AWAITING_OPERATOR_DECISION].includes(run.state));
...
function automaticCoreSurfaceReviewHasPriority(windowContext = {}) {
  ...
  return Boolean(
    ...
    run?.sessionContextInit?.state === "READY"
  );
}
...
function renderSnapshot(snapshot, { preserveInputs = false } = {}) {
  ...
  elements.timeoutStatus.textContent = run?.timeoutSuspended
    ? `Suspenderad · Nano wall ${Math.round(Number(config.nanoWallTimeoutMs || NANO_WALL_TIMEOUT_MS) / 1000)} s`
    : `Normal · Nano wall ${Math.round(Number(config.nanoWallTimeoutMs || NANO_WALL_TIMEOUT_MS) / 1000)} s`;
  ...
  if (!preserveInputs) {
    ...
    elements.nanoMandateVersion.value = config.nanoMandateVersion || "nano-core-v7";
    elements.targetMandate.value = config.targetMandate || "";
    elements.targetMandateVersion.value = config.targetMandateVersion || "target-core-v6";
  ...
  if (coreReviewPriority) {
    ...
    if (run?.state === STATES.ASSESSING && run.pendingNanoRequest?.status === "PENDING") {
...
function buildNanoSystemPrompt(config) {
  return `You are the local continuity controller for EIC Autonom Agent.
...
Keep the base session compact and immutable. Per-task prompts carry only owner references, active binding and the latest material delta.
...
async function promptNano(prompt, responseConstraint = null, telemetryOptions = null) {
  ...
  const nanoWallTimeoutMs = Number.isFinite(configuredNanoWallTimeoutMs) &&
    configuredNanoWallTimeoutMs >= 60_000
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (typeof session.promptStreaming === "function") {
        ...
        let overrun = "";
        await withNanoWallDeadline((async () => {
          ...
          while (true) {
            ...
            // Output ceilings, material-output idle deadline and wall-clock deadline
            // are independent. A host can no longer stream heartbeat-like duplicates
            // for the full 30-minute wall timeout without a terminal result.
            if (output.length > NANO_MAX_OUTPUT_CHARS) {
              overrun = `Nano-output överskred ${NANO_MAX_OUTPUT_CHARS} tecken (${output.length}).`;
            } else if (chunkCount > NANO_MAX_OUTPUT_CHUNKS) {
              overrun = `Nano-output överskred ${NANO_MAX_OUTPUT_CHUNKS} chunks (${chunkCount}).`;
            }
            if (overrun) break;
        ...
        if (overrun) {
          // Non-fatal by default: an over-long stream usually means a complete decision
          // object followed by prose, not a corrupt one. Only a genuinely unparsable
          // output is escalated.
          const salvaged = extractFirstJsonObject(output);
          if (!salvaged) throw new NanoOutputOverrunError(overrun);
          output = salvaged;
          overrunRecovered = overrun;
        }
      } else {
        try {
          output = String(await withNanoWallDeadline(
      ...
      // v0.7.0: the 80 % high-watermark guard was dead code on the field host because
      // `contextUsage`/`contextWindow` were never exposed (both null in the exported
      // telemetry), so the shared base session grew without any brake at all. A local
      // character accumulator now bounds the shared path even on a silent host.
      if (!task.owned) {
      ...
      lastError = error;
      if (error instanceof NanoWallTimeoutError) {
        markBaseSessionStale(
          "NANO_WALL_TIMEOUT",
      ...
      const canRetry = !(error instanceof NanoWallTimeoutError) &&
        attempt + 1 < maxAttempts && recoverableTaskSessionError(error) && task?.owned;
...
function normalizeDecision(value) {
  ...
  return {
    analysisMode: Object.values(NANO_ANALYSIS_MODES).includes(value?.analysisMode)
      ? value.analysisMode
      : NANO_ANALYSIS_MODES.CONTINUATION_ANALYSIS,
    ...
    alternatives: (value?.alternatives || []).map((item) => sanitizeText(item, 2000)).filter(Boolean),
    ...
    trackControl: normalizeTrackControl(value?.trackControl, {
      baselinePresent: Boolean(value?.trackControl?.baselinePresent)
    }),
    operatorCandidate: value?.operatorCandidate && typeof value.operatorCandidate === "object"
      ? {
          sourceClass: "NANO_PROPOSED",
          triggerType: ["HUMAN_ACTION_CANDIDATE", "OPERATOR_PROXY_REQUIRED"].includes(value.operatorCandidate.triggerType)
            ? value.operatorCandidate.triggerType
            : "NONE",
          actionCode: sanitizeText(value.operatorCandidate.actionCode, 120),
          exactTarget: sanitizeText(value.operatorCandidate.exactTarget, 1200),
          proposedAction: sanitizeText(value.operatorCandidate.proposedAction, 1600),
          expectedEffect: sanitizeText(value.operatorCandidate.expectedEffect, 1600),
          ownerSurface: sanitizeText(value.operatorCandidate.ownerSurface, 400),
          ownerEvidenceLocator: sanitizeText(value.operatorCandidate.ownerEvidenceLocator, 800),
          reversibility: value.operatorCandidate.reversibility || "UNKNOWN",
          rollbackPath: sanitizeText(value.operatorCandidate.rollbackPath, 1200),
          readbackPlan: sanitizeText(value.operatorCandidate.readbackPlan, 1200),
          destructivenessLevel: Math.max(1, Math.min(10, Number(value.operatorCandidate.destructivenessLevel || value.destructivenessLevel || 1))),
          destructivenessRationale: sanitizeText(value.operatorCandidate.destructivenessRationale || value.destructivenessRationale, 1200),
          humanAuthorityClass: value.operatorCandidate.humanAuthorityClass || "UNKNOWN",
          materialAmbiguity: value.operatorCandidate.materialAmbiguity || "UNKNOWN"
        }
      : null
  };
}
...
async function processPendingCoreSurfaceReview(snapshot) {
  ...
  if (state.coreSurfaceReviewInFlight || state.modelBusy || state.activeNanoRequestId) return;
  ...
  try {
    ...
    const prompt = buildCoreSurfaceReviewPrompt({
      ...
      continuity: projectContinuity(current.continuity || {}, { maxChars: 6_000 }),
      captureSummary: current.window?.sessionCaptureSummary || {},
      memorySummary: current.window?.sessionMemorySummary || {}
    });
    const result = await promptNano(prompt, CORE_SURFACE_REVIEW_RESPONSE_SCHEMA, {
      withTelemetry: true,
...
async function processPendingNano(snapshot) {
  ...
  if (state.modelBusy || state.activeNanoRequestId) return;
  ...
  const buildAt = (attempt) => buildNanoDecisionPromptDetailed({
    run,
  ...
  const inputDigest = await sha256Hex(decisionPrompt);
  ...
  const heartbeat = async () => {
    if (!claimId || heartbeatBusy) return;
    heartbeatBusy = true;
    try {
    ...
    const runInference = () => promptNano(decisionPrompt, DECISION_SCHEMA, {
      withTelemetry: true,
      onProgress: async (progress) => {
        ...
        const now = Date.now();
        if (chunkCount === 1 || now - lastProgressHeartbeatAt >= 2_000) {
...
${JSON.stringify(sanitizeText(result.text, 8000))}`;
      const repairResult = await promptNano(repairPrompt, DECISION_SCHEMA, {
        withTelemetry: true,
        disableFreshTaskRetry: true,
        onProgress: async (progress) => {
    ...
    let decision = normalizeDecision(parsedDecision);
    ...
    if (Number(request.repairAttempt || 0) > 0 && request.priorDecisionFields) {
    ...
    const preGrounding = validateNanoDecisionGrounding(decision, {
      analysisMode,
      continuityProjection,
      observation
    });
    if (!preGrounding.valid && preGrounding.errors.includes("TAKEOVER_CONTEXT_EMPTY")) {
    ...
    const localGrounding = validateNanoDecisionGrounding(decision, {
      analysisMode,
      continuityProjection,
      observation
    });
    ...
    const response = await command("NANO_DECISION", {
      requestId: request.requestId,
      claimId,
      analysisMode,
      source: "NANO",
    ...
    if (claimId) {
      try {
        const failed = await command("NANO_FAILURE", {
          ...
          errorCode: error?.name || "NANO_PIPELINE_ERROR",
          errorDetail: errorMessage(error),
          repairUsed: Number(request.repairAttempt || 0) > 0
        });
        renderSnapshot(failed, { preserveInputs: true });
      } catch {
        // Ownern kan ha avvisat en stale/expired failure. Ingen falsk status skrivs.
      }
    }
    throw error;
  } finally {
...
async function autostartClick() {
  ...
  applyScenarioPreset(plan.configPatch.quickProfileId);
  ...
  try {
    let snapshot = await command("SAVE_CONFIG", { config: preparedConfig });
    renderSnapshot(snapshot, { preserveInputs: true });

    snapshot = await command("LINK_ACTIVE_TAB");
    renderSnapshot(snapshot, { preserveInputs: true });
