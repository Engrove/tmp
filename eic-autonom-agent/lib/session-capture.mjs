import { nowIso, sanitizeText, sha256Hex, stableStringify } from "./common.mjs";
  ...
  try {
    ...
    for (let sweep = 0; sweep < maxSweeps; sweep += 1) {
      ...
      for (const message of visible) {
        const key = message.sourceMessageId || `${message.role}:${message.ordinal}:${cleanText(message.text, 200)}`;
