...
export function createCdpSession({
  ...
  return {
    ...
    tabId: Number.isInteger(Number(surface?.tabId)) ? Number(surface.tabId) : null,
...
export function normalizeCdpSession(value, {
  ...
  return {
    ...
    tabId: Number.isInteger(Number(source.tabId ?? surface?.tabId))
      ? Number(source.tabId ?? surface?.tabId)
      : null,
...
export function cdpSessionMatchesSurface(sessionValue, surface) {
  ...
  return Boolean(
    Number.isInteger(Number(surface?.tabId)) &&
...
function assertAttachPreconditions({ profile, surface, permissionGranted }) {
  ...
  if (!Number.isInteger(Number(surface?.tabId))) throw new Error("CDP_TARGET_TAB_REQUIRED");
...
export async function attachBoundedCdp(chromeApi, {
  ...
  await chromeApi.debugger.attach({ tabId: Number(surface.tabId) }, CDP_PROTOCOL_VERSION);
...
export async function detachBoundedCdp(chromeApi, sessionValue, {
  ...
  try {
    if (Number.isInteger(Number(session.tabId)) && chromeApi?.debugger?.detach) {
      await chromeApi.debugger.detach({ tabId: Number(session.tabId) });
    }
  } catch (error) {
