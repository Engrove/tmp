...
export async function buildTurnObject({
  ...
  const object = {
    ...
    responseContract: {
      ...
      nextActorLine: "EIC_NEXT_ACTOR: <AGENT|OPERATOR_ACTION|OPERATOR_DECISION|EXTERNAL_SYSTEM|NONE>",
...
export function locateTargetTrailer(text, { allowTurnless = false } = {}) {
  ...
  const scan = indexedLines.slice(-30);
  const markerCounts = {
    turn: scan.filter((entry) => /^EIC_TURN:\s*/i.test(entry.text)).length,
    next: scan.filter((entry) => /^EIC_NEXT:\s*/i.test(entry.text)).length,
    evidence: scan.filter((entry) => /^EIC_COMPLETION_EVIDENCE:\s*/i.test(entry.text)).length,
    nextActor: scan.filter((entry) => /^EIC_NEXT_ACTOR:\s*/i.test(entry.text)).length,
    status: scan.filter((entry) => /^EIC_AUTONOMY:\s*/i.test(entry.text)).length
  };
  if (markerCounts.next !== 1 || markerCounts.evidence !== 1 ||
      markerCounts.nextActor !== 1 || markerCounts.status !== 1 ||
      (!allowTurnless && markerCounts.turn !== 1) || (allowTurnless && markerCounts.turn > 1)) {
  ...
  if (candidates.length !== 1) {
    return {
      valid: false,
      reason: candidates.length ? "PROTOCOL_AMBIGUOUS" : "PROTOCOL_TRAILER_NOT_EXACT",
      stripped,
      block: null
    };
  }
  return { valid: true, reason: "OK", stripped, block: candidates[0] };
}
