...
/**
 * The operator retry re-arms the chain from its first phase. It is bounded so a
 * structurally broken target cannot be retried indefinitely from the UI.
 */
...
export function retrySessionContextInit(value, { now = Date.now() } = {}) {
  ...
  return { ok: true, value: next, reason: "RETRY_ARMED" };
}
...
export function sessionContextInitOverlay(value) {
  ...
  const map = {
    ...
    [SESSION_CONTEXT_INIT_STATE.FAILED]: {
      title: "Baselinefrågan kunde inte levereras",
