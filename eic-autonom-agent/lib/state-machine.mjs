...
import { createSessionContextInit } from "./session-context-init.mjs";
...
const ALLOWED = Object.freeze({
  ...
  [STATES.AWAITING_OPERATOR_ACTION]: new Set([
    STATES.RECOVERING, STATES.RUNNING, STATES.WAITING_FOR_RESPONSE, STATES.CONTINUING, STATES.STOPPED
  ]),
...
export function transitionRun(run, to, {
  ...
  if (!force && run.state !== to && !canTransition(run.state, to) && !safetyEscape) {
