...
export function normalizeBrowserAction(value) {
  ...
  const target = {
    tabId: Number(targetValue.tabId),
  ...
  if (!Number.isInteger(target.tabId)) throw new Error("BROWSER_ACTION_TAB_ID_INVALID");
...
function sameIdentity(target, surface) {
  return Number(target.tabId) === Number(surface?.tabId) &&
