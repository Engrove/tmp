...
export function detachRunFromTab(runValue, linkedRecord = {}, { now = Date.now() } = {}) {
  ...
  const tabId = Number(linkedRecord?.tabId);
  if (!run || !Number.isInteger(tabId) || Number(run.targetTabId) !== tabId ||
...
export function reattachRunToTab(runValue, {
  ...
  const run = deepClone(runValue);
  if (!runIsDetached(run) || !Number.isInteger(Number(tabId))) {
